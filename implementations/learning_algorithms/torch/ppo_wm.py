...
from interfaces.network import Policy_Index_Network, Value_Network
...
def gather_sum(values: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
    """
    Args:
        values: Tensor of shape (batch_size, episodic_memory_size, 1)
        indices: Tensor of shape (batch_size, sequence_length, working_memory_size)
                 with values in [0, episodic_memory_size - 1]
    Returns:
        Tensor of shape (batch_size, sequence_length, 1)
    """
    B, S, W = indices.shape
    
    # Create a batch index tensor shaped (B, 1, 1) to broadcast across S and W
    batch_idx = torch.arange(B, device=values.device).view(B, 1, 1)
    
    # Index directly. This returns a tensor of shape (batch_size, sequence_length, working_memory_size, 1)
    gathered_values = values[batch_idx, indices]
    
    # Sum over the working_memory_size dimension (dim=2)
    return gathered_values.sum(dim=2)


class PPO(RL_Index_Learner, Safe_nn_Module):

    def __init__(self, policy_model: Policy_Index_Network, value_model: Value_Network, device, persistence_path=None, minibatch_size=8, aux_coef=None):
        """
        ...
        Safe_nn_Module.__init__(self, 
            ...
            modules={"ppo_wm_learner": self.optimizer}
        )
    ...
    def learn(self, 
        ...
        """
        obs: np array of shape (batch_size, episodic_memory_size, ...)
        indices: np array of shape (batch_size, context_length + 1, woring_memory_size)
        ...
        b_indices = convert_np_array_to_float_tensor(indices, self.device).long()
        ...
        with torch.no_grad():
            ...
            for start in range(0, batch_size, self.minibatch_size):
                ...
                mb_logprob, _ = self.policy_model.get_log_probability(
                    context=mb_obs,
                    indices=mb_indices[:, :-1, ...],
                    selected_action=mb_actions[:, 1:, ...],
                    valid_actions=mb_valid_actions,
                )
                logprobs_list.append(mb_logprob)

                mb_sub_values = self.value_model.get_value(mb_obs)
                mb_values = gather_sum(mb_sub_values, mb_indices)
        ...
        for epoch in range(self.update_epochs):
            ...
            for start in range(0, batch_size, self.minibatch_size):
                ...
                if self.aux_coef is not None:
                    ...
                    mb_indices = b_indices[mb_inds, :, ...]
                    mb_actions = b_actions[mb_inds, 1:, ...]
                    mb_valid_actions = b_valid_actions[mb_inds, ...] if b_valid_actions is not None else None
                    
                    mb_newlogprob, mb_entropy, aux_unsum_loss = self.policy_model.get_log_probability_with_aux_loss(
                        context=mb_obs,
                        indices=mb_indices,
                        selected_action=mb_actions,
                        valid_actions=mb_valid_actions
                    )
                    aux_loss = masked_mean(aux_unsum_loss, b_aux_masks[mb_inds, ...])

                    mb_sub_values = self.value_model.get_value(mb_obs)
                    mb_newvalue = gather_sum(mb_sub_values, mb_indices[:, :-1, ...])
                else:
                    mb_obs = b_obs[mb_inds, ...]
                    mb_indices = b_indices[mb_inds, :-1, ...]
                    mb_actions = b_actions[mb_inds, 1:, ...]
                    mb_valid_actions = b_valid_actions[mb_inds, ...] if b_valid_actions is not None else None
                    ...
                    mb_sub_values = self.value_model.get_value(mb_obs)
                    mb_newvalue = gather_sum(mb_sub_values, mb_indices)
