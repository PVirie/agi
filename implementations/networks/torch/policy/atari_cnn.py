...
class Atari_Core(Policy_Network, nn.Module, Safe_nn_Module):
    ...
    def __init__(self, action_size, position_size, width, height, channel, hidden_size, layers, history_steps=0, max_temporal_len=32, device=None, persistence_path=None):
        ...
        self.packed_action_size = 1 + 1 + self.position_size  # ext_flag + action + position
        ...
        self.cnn = ImpalaCNN(output_dims=hidden_size, input_channels=channel, width=width, height=height, depths=[32, 64, 128, hidden_size][:layers])
    ...
    def __compute(self, context):
        ...
        features = self.cnn(torch.reshape(image_part, (batch_size * context_size, self.channel, self.height, self.width)))
        features = torch.reshape(features, (batch_size, context_size, -1))  # (batch_size, context_size, feature_size)

        return features, last_position
    ...
    def get_action(self, context, valid_actions=None):
        ...
        batch_size = context.size(0)
        features, last_position = self.__compute(context)
    ...
    def get_log_probability(self, context, selected_action, valid_actions=None):
        ...
        return torch.stack([
            log_prob_flag, log_prob_action, log_prob_position
        ], dim=-1), torch.stack([
            entropy_flag, entropy_action, entropy_position
        ], dim=-1)
    ...
    def unpack_action(self, packed_action):
        ...
        content = np.zeros((packed_action.shape[0], self.content_size), dtype=float)
