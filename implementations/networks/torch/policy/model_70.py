...
class Policy_Core(Base_Policy_Core):
    ...
    def __init__(self, 
        ...
        self.position_size = self.goal_size + 2 # sub-goal, alpha, nu
        ...
        # Policy backbone: goal, inv, obs -> hidden
        ...
        # Sub-result: inv, obs -> embedding + hidden
        ...
        # Alpha: goal, sub_goal, inv, obs -> [0, 1]
        self.alpha_module = ResNet(
            output_dims=1, 
            input_dims=predict_input_dim,
            hidden_dims=hidden_size, layers=layers
        )

        # Action head: hidden -> action
    ...
    def reset_parameters(self):
        ...
        self.nu_module.reset_parameters()
        self.alpha_module.reset_parameters()
    ...
    def compute(self, context):
        ...
        last_subgoal = context[:, :, (1 + 1 + 1): (1 + 1 + 1 + self.goal_size)]  # (batch_size, context_size, goal_size)
        last_alpha = context[:, :, (1 + 1 + 1 + self.goal_size): (1 + 1 + 1 + self.goal_size + 1)]  # (batch_size, context_size, 1)
        last_nu = context[:, :, (1 + 1 + 1 + self.goal_size + 1): (1 + 1 + 1 + self.goal_size + 2)]  # (batch_size, context_size, 1)
        ...
        base_int_logits = self.head_int(base_output)  # (batch_size, context_size, int_action_size)
        base_ext_logits = self.head_ext(base_output)  # (batch_size, context_size, ext_action_size)
        
        # Calculate nu factor

        nu_input = torch.concat([last_subgoal_embedded, inv_embedded, obs_features], dim=-1)  # (batch_size, context_size, vec_dim)
        nu_logits = self.nu_module(lwlv_input)  # (batch_size, context_size, 1)
        probs_nu = Bernoulli(logits=nu_logits)
        sampled_nu = probs_nu.sample()  # (batch_size, context_size, 1)
        log_probs_nu = probs_nu.log_prob(sampled_nu)  # (batch_size, context_size, 1)

        # Calculate new sub-goal
        ...
        using_subgoal_logits = sampled_nu * subgoal_logits + (1 - sampled_nu) * last_subgoal_embedded  # (batch_size, context_size, goal_size * embedding_dim)

        # Calculate action from sub-goal

        lwlv_input = torch.concat([using_subgoal_logits, inv_embedded, obs_features], dim=-1)  # (batch_size, context_size, vec_dim)
        lwlv_output = self.backbone(lwlv_input)  # (batch_size, context_size, hidden_size)

        cul_int_logits = self.head_int(lwlv_output)  # (batch_size, context_size, int_action_size)
        cul_ext_logits = self.head_ext(lwlv_output)  # (batch_size, context_size, ext_action_size)

        # Calculate alpha
        
        alpha_input = torch.concat([base_output, lwlv_output], dim=-1)  # (batch_size, context_size, hidden_size * 2)
        alpha_logits = self.alpha_module(alpha_input)  # (batch_size, context_size, 1)
        probs_alpha = Bernoulli(logits=alpha_logits)
        sampled_alpha = probs_alpha.sample()  # (batch_size, context_size, 1)
        log_probs_alpha = probs_alpha.log_prob(sampled_alpha)  # (batch_size, context_size, 1)

        # Calculate final logits

        ## train use probability, inference use sampled value
        using_alpha = sampled_alpha * sampled_nu  + last_alpha * (1 - sampled_nu)
        int_logits = using_alpha * cul_int_logits + (1 - using_alpha) * base_int_logits
        ext_logits = using_alpha * cul_ext_logits + (1 - using_alpha) * base_ext_logits

        next_sub_goal = torch.argmax(using_subgoal_logits.view(batch_size, context_size, self.goal_size, -1), dim=-1)  # (batch_size, context_size, goal_size)

        positions = torch.concat([next_sub_goal, using_alpha, sampled_nu], dim=-1)  # (batch_size, context_size, position_size)

        # Calculate aux loss

        ## make nu sparse
        nu_loss = log_probs_nu.mean()  # encourage nu to be 0, so that the model will only use sub-goal when necessary

        aux_loss = nu_loss

        return int_logits, ext_logits, positions, aux_loss
    ...
    def get_action(self, context, valid_actions=None):
        ...
        action = np.concatenate([
            ...
            np.zeros((batch_size, context_size, self.content_size), dtype=np.float32)
        ], axis=-1)
