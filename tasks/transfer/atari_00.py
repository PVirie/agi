...
if __name__ == "__main__":
    ...
    first_init = False
    source_path = f"{APP_ROOT}/experiments/transfer/atari_base_{args.scale}_aux{args.aux_coef}"
    experiment_path = f"{APP_ROOT}/experiments/transfer/atari_00_{args.scale}_aux{args.aux_coef}"
    # clear the experiment path
    if not os.path.exists(experiment_path):
        first_init = True
        os.makedirs(experiment_path, exist_ok=True)
        # copy only the model weight (all file start with base_core_*)
        # walk directory and copy files, keeping the directory structure
        for root, dirs, files in os.walk(source_path):
            for file in files:
                if file.startswith("base_core_"):
                    src_file = os.path.join(root, file)
                    dst_file = os.path.join(experiment_path, "parameters", file)
    ...
    game_ids=["ALE/Breakout-v5"] * 32 + ["ALE/Galaxian-v5"] * 32
    ...
    if first_init:
        # now reset only the mapping
        policy_core.conv_layers.reset_parameters()
        policy_core.mapping_core.reset_parameters()
