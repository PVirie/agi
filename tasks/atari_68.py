...
if __name__ == "__main__":
    ...
    experiment_path = f"{APP_ROOT}/experiments/atari_68"
    if args.reset:
    ...
    policy_core = Policy_Core(
        ...
        position_size=position_size, working_memory_size=working_memory_size,
    ...
    memory = Memory(
        sizes=(1, 1, 1, position_size, policy_core.content_size),
        max_slot_size=512
    )
    agent = model_68.Model_68(
        policy_model=policy_core, value_model=value_core,
        trainer=ppo_learner,
        index_collector=Collector(max_history=0),
